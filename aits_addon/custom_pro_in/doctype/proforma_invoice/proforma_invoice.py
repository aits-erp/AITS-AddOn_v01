...
class ProformaInvoice(SellingController):
    ...
    # def validate_so_remaining_qty(self):
    ...
    #             WHERE sales_order_item = %s
    #             AND parent != %s
    #             AND docstatus != 2

    #         """, (row.sales_order_item, self.name))[0][0] or 0

    #         remaining_qty = float(so_qty) - float(used_qty)

    #         if row.qty > remaining_qty:
    #             frappe.throw(f"""
    #                 <b>Quantity exceeded</b><br><br>
    #                 Item: {row.item_code}<br>
    #                 Sales Order Qty: {so_qty}<br>
    #                 Already Used: {used_qty}<br>
    ...
    def validate_so_remaining_qty(self):
        for row in self.items:
            ...
            # Already Used Qty (ignore cancelled)
            used_qty = frappe.db.sql("""
                SELECT SUM(qty)
                FROM `tabProforma Invoice Item`
                WHERE sales_order_item = %s
    ...
    def before_validate(self):
        pass
        self.flags.ignore_so_setting = True
        self.flags.ignore_so_qty = True
    ...
    def validate(self):
        ...
        self.validate_so_remaining_qty()
        ...
        for item in self.items:
            if item.sales_order_item:
                ...
                if (used_qty + item.qty) > so_qty and self.docstatus != 2:
                    # Remove this throw if you want unrestricted qty
                    frappe.throw(
                        f"Row #{item.idx}: Qty exceeds Sales Order qty ({so_qty}) for {item.item_code}"
                    )
    ...
    #                 frappe.throw(
    #                     _("Row #{0}: Qty {1} exceeds Sales Order qty {2}")
    #                     .format(item.idx, used_qty + item.qty, so_qty)
    #                 )

    def set_has_unit_price_items(self):
        # Prevent AttributeError in ERPNext hooks
        self.has_unit_price_items = 0
        for item in self.items:
            if item.rate and item.rate > 0:
                self.has_unit_price_items = 1
                break
    ...
    def check_sales_order_quantity(self):
        for row in self.items:
            ...
            data = frappe.db.sql("""
                ...
                INNER JOIN `tabProforma Invoice` pi
                    ON pi.name = pii.parent
                WHERE 
                    pii.sales_order_item = %s
            """, (row.sales_order_item,), as_dict=True)

            used_qty = 0

            for d in data:

                # ✅ ONLY count submitted invoices
                if d.docstatus != 1:
                    continue
                ...
                # ✅ Ignore old/amended docs
                if self.amended_from and d.name == self.amended_from:
                    continue

                # ✅ Ignore documents that are amended BY this one
                if d.amended_from == self.name:
                    continue

                used_qty += float(d.qty or 0)

            remaining_qty = float(sales_order_qty) - float(used_qty)

            frappe.msgprint(
                f"<b>DEBUG INFO</b><br>"
                f"Item: {row.item_code}<br>"
                f"Sales Order Qty: {sales_order_qty}<br>"
                f"Already Used: {used_qty}<br>"
                f"Remaining: {remaining_qty}",
                indicator="blue",
            )

            if float(row.qty) > float(remaining_qty):
                frappe.throw(
                    f"Row #{row.idx}: Qty {row.qty} exceeds remaining Sales Order qty "
                    f"{remaining_qty} for {row.item_code}"
                )
    ...
    def check_nextdoc_docstatus(self):
        """Block cancel if Draft Sales Invoice exists"""
        ...
        linked_invoices = frappe.db.sql_list(
            ...
            WHERE si_item.against_proforma_invoice = %s
            AND si.docstatus = 0
            """,
            (self.name,)
        )
...
@frappe.whitelist()
def get_remaining_so_qty(so_item, current_pi=None):

    if not so_item:
        return 0

    so_qty = frappe.db.get_value("Sales Order Item", so_item, "qty") or 0

    used_qty = frappe.db.sql("""
        SELECT COALESCE(SUM(pi_item.qty), 0)
        FROM `tabProforma Invoice Item` pi_item
        JOIN `tabProforma Invoice` pi ON pi.name = pi_item.parent
        WHERE pi.docstatus < 2
        AND pi_item.prevdoc_detail_docname = %s
        AND (%s IS NULL OR pi.name != %s)
    """, (so_item, current_pi, current_pi))[0][0] or 0

    remaining = float(so_qty) - float(used_qty)
