...
frappe.ui.form.on("Proforma Invoice", {
    ...
    refresh: function (frm) {
        ...
        // Make Taxes grid editable inline
        if (frm.fields_dict.taxes) {
            frm.fields_dict.taxes.grid.editable_rows = true;

            // ensure columns are editable
            frm.fields_dict.taxes.grid.editable_fields = [
                { fieldname: "charge_type", columns: 2 },
                { fieldname: "account_head", columns: 4 },
        ...
        // Stop opening popup dialog
           const grid = frm.fields_dict.taxes?.grid;
        if (!grid) return;

        // Disable popup forever
        grid.open_grid_row = function () {
            return;
        };

        grid.editable_rows = true;
        grid.refresh();
...
frappe.ui.form.on("Proforma Invoice Item", {
        ...
        frappe.db.get_value("Sales Order Item", row.so_detail, ["qty"], (r) => {
            ...
            frappe.call({
                ...
                callback: function(res){
                    let used_qty = 0;
                    res.message.forEach(i => used_qty += i.qty);

                    let remaining = r.qty - used_qty;
   ...
   frappe.call({
      ...
      callback: function(res){
         let used = res.message.reduce((a,b)=>a+b.qty,0);

         frappe.db.get_value("Sales Order Item", row.so_detail, "qty", (r)=>{
            let remaining = r.qty - used;
            row.qty = remaining;
            frm.refresh_field("items");
         });
      }
...
frappe.ui.form.on("Proforma Invoice", {
    onload(frm) {
        // if taxes table is empty → create one default row
    ...
    refresh(frm) {
        // keep grid inline (no popup)
        const grid = frm.fields_dict.taxes?.grid;
        if (!grid) return;

        grid.editable_rows = true;

        // optional: stop popup dialog
        grid.open_grid_row = function () {
            return;
        };
...
function set_remaining_qty_only(frm, cdt, cdn) {
    ...
    frappe.call({
        method: "customproformainvoice.customproformainvoice.doctype.proforma_invoice.proforma_invoice.get_remaining_so_qty",
        args: {
            so_item: row.prevdoc_detail_docname,
            current_pi: frm.doc.name || ""
        },
        callback: function(r) {

            if (!r.message) return;

            let remaining = flt(r.message);

            if (remaining <= 0) {
                frappe.msgprint({
                    title: "Limit Reached",
                    indicator: "red",
                    message: `No remaining quantity for this Sales Order line`
                });
            ...
            // AUTO SET qty = remaining
            frappe.model.set_value(cdt, cdn, "qty", remaining);
        }
    });
}

function limit_qty_to_remaining(frm, cdt, cdn) {
        ...
        callback: function(r) {
            ...
            if (row.qty > remaining) {
                ...
                frappe.msgprint({
                    ...
                    indicator: "red",
                    message: `Maximum allowed quantity is only ${remaining}`
                });
